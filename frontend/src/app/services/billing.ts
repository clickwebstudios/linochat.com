...
export const billingService = {
  ...
  cancelSubscription: (reason?: string): Promise<void> =>
    api.delete('/billing/subscription', { data: reason ? { reason } : {} }).then(() => undefined),
  resumeSubscription: (): Promise<void> =>
    api.post('/billing/subscription/resume').then(() => undefined),
  saveDowngradeSelection: (data: { keep_agent_ids: number[]; keep_project_ids: number[] }): Promise<void> =>
    api.post('/billing/downgrade-selection', data).then(() => undefined),
  upgradePaidPlan: (data: { plan_name: string; billing_cycle: string }): Promise<void> =>
    api.post('/billing/upgrade', data).then(() => undefined),
