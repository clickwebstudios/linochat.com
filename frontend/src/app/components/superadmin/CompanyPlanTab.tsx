          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="w-full bg-muted rounded-full h-2">
                <div className="bg-primary h-2 rounded-full" style={{ width: `${Math.min((companyAgentsCount / (company.plan === 'Enterprise' ? 100 : company.plan === 'Pro' ? 25 : 5)) * 100, 100)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="w-full bg-muted rounded-full h-2">
                <div className="bg-secondary h-2 rounded-full" style={{ width: `${Math.min((companyProjectsCount / (company.plan === 'Enterprise' ? 50 : company.plan === 'Pro' ? 20 : 3)) * 100, 100)}%` }} />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-muted-foreground">Tickets This Month</span>
            ...
            <div className="pt-2">
              {company.plan !== 'Enterprise' && (
                <Button size="sm" className="w-full bg-primary" disabled={isArchived}>
        ...
        <Card className="md:col-span-3">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Change the subscription plan for {editedCompanyName}</p>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                const borderColor = plan.color === 'gray' ? 'border-border' : plan.color === 'green' ? 'border-green-500' : plan.color === 'blue' ? 'border-primary' : 'border-secondary';
                const badgeBg = plan.color === 'gray' ? 'bg-muted text-foreground' : plan.color === 'green' ? 'bg-green-100 text-green-700' : plan.color === 'blue' ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary';
                const isDowngrade = (['Free', 'Starter', 'Pro', 'Enterprise'].indexOf(plan.name) < ['Free', 'Starter', 'Pro', 'Enterprise'].indexOf(company.plan));
                return (
                  <div key={plan.name} className={`relative rounded-xl border-2 p-4 transition-all ${isCurrent ? borderColor + ' bg-muted/50' : 'border-border hover:border-border'} ${plan.popular && !isCurrent ? 'ring-1 ring-primary/20' : ''}`}>
                    ...
                    <div className="text-center pt-2 pb-3">
                      ...
                      <div className="mt-1.5">
                        ...
                        {plan.period !== 'forever' && plan.period !== 'contact us' && (
                          <span className="text-xs text-muted-foreground ml-1">/{plan.period.split('/')[1]}</span>
                        )}
                      </div>
                    ...
                    <div>
                      {isCurrent ? (
                        ...
                        <Button size="sm" className="w-full text-xs bg-primary hover:bg-primary/90" disabled={isArchived}>
